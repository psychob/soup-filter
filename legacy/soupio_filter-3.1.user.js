...
var _sif = {
 ...
 pre_req : function ()
 {
  if (!_sif._canYouRunIt)
   return ;
  console.log('Check if SOUP.IO');
   
  // najpierw sprawdźmy czy jesteśmy w domenie soup.io
  if (/\.soup\.io/.test(document.location.host))
  {
   _sif._canYouRunIt = true;
   console.log("SOUP.IO status: true");
  } else
  {
   // jeśli jednak nasz soup nie jest w domenie soup.io/
   // sprawdzamy jeszcze czy istnieje zmienna SOUP
   if (typeof unsafeWindow.SOUP == "undefined")
   {
    _sif._canYouRunIt = false;
    console.log("SOUP.IO status: false");
   } else
   {
    _sif._canYouRunIt = true;
    console.log("SOUP.IO status: true");
 ...
 addElement : function (elementId, tocont)
 {
  ...
  img.setAttribute("alt", elementId);
